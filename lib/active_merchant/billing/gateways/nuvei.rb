  ...
  module Billing
    class NuveiGateway < Gateway
      ...
      def purchase(money, payment, options = {})
        post = {}
        build_post_data(post, :payment)
        add_invoice(post, money, options)
        add_payment(post, payment)
        add_address(post, payment, options, :billingAddress)
        add_address(post, payment, options, :shippingAddress)
        add_customer_ip(post, options)
      ...
      def credit(money, payment, options = {})
        ...
        build_post_data(post, :general_credit)
        add_invoice(post, money, options)
        add_payment(post, payment, :cardData)
        add_address(post, payment, options)
        add_customer_ip(post, options)
        # The card details should be nested under cardData
        card_details = post[:cardData].delete(:card)
        post[:cardData].merge!(card_details)
      ...
      def add_payment(post, payment, key = :paymentOption)
        if payment.is_a?(CreditCard)
          post[key] = { card: credit_card_hash(payment) }
        else
          post[key] = { card: { cardToken: payment } }
        end
      end
      ...
      def message_from(response)
        response[:transactionStatus] || response[:reason]
      end
    end
  end
end
